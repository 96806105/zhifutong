...
转人工不只是「发一张模拟工单」——可对接**飞书机器人**完成真实双向闭环：

```
网页转人工 ──► 工单 + 求助信息实时推送飞书群
    ▲                    │
    │                    ▼
网页实时收到 ── 回写会话 ◄── 人工客服在飞书直接回复
```

- 推送与回写均走飞书官方 **WebSocket 长连接**，无需公网回调，本地可完整演示
- 人工回复自动回写会话，网页端实时出现带 **「人工回复」** 绿色标识的气泡
- 回复可精确绑定工单（`TKT-XXXX 回复内容`），自动关闭工单

**使用方法：** 开通飞书企业自建应用 → 配置 `.env` 的 `LARK_*`（10 分钟，全程无需公网）→ 启动 Web 服务 + `python scripts/feishu_bot.py`。
完整图文步骤见 [`docs/05-飞书人工接线.md`](docs/05-飞书人工接线.md)。
...
| `05-飞书人工接线.md` | 真实人工双向闭环：应用开通 / 配置 / 演示 |
